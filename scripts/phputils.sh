...
pecl uninstall xdebug
pecl install xdebug
cp $(brew --prefix php@8.3)/pecl/20210902/xdebug.so /tmp/php8.3-xdebug.so
...
pecl uninstall xdebug
pecl install xdebug
cp $(brew --prefix php)/pecl/20230831/xdebug.so /tmp/php8.4-xdebug.so
ln -s $(brew --prefix pcre2)/include/pcre2.h $(brew --prefix php)/include/php/ext/pcre/pcre2.h
pecl uninstall pcov
pecl install pcov
cp $(brew --prefix php)/pecl/20230831/pcov.so /tmp/php8.4-pcov.so
...
cp /tmp/php8.3-xdebug.so $(brew --prefix php@8.3)/pecl/20210902/xdebug.so
...
cp /tmp/php8.4-pcov.so $(brew --prefix php)/pecl/20230831/pcov.so
